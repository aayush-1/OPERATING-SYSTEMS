...
int totalthreads=NUM_BLOCKS*NUM_THREADS;
if(totalthreads>=N)
{
if (index1 < N) 
{
for(int i=0;i<N;i++)
{
*(c+index1*N+i) = *(a+index1*N+i) + *(b+index1*N+i);
...
int main (int argc, char **argv) {
  ...
  for(i=0;i<N;i++) {
	...
	{
          printf("error: h_C[%d][%d]!=C[%d][%d] (%d, %d)\n",i,j,i,j,*(h_C+i*N+j),C[i][j]);
	}
